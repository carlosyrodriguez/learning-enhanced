...
export async function POST(request: NextRequest) {
  try {
    ...
    const cacheKey = `lessons-${project.name}-${project.language}-${JSON.stringify(
      project.files?.slice(0, 3),
    )}`;
...
Focus on practical, typeable code examples that build understanding progressively.`;
    ...
    try {
      ...
      try {
        ...
        // Sanitize and format the response to ensure consistency
        ...
        // Provide a robust fallback lesson if parsing fails
        lessons = [
          {
            id: "project-overview",
...
// - Real-world implementation`,
            explanation: `${project.name} is a ${
              project.language
            } project that showcases modern development practices. By studying its structure and implementation, you'll learn valuable patterns and techniques used in professional software development.`,
            keyPoints: [`Built with ${project.language}`, "Demonstrates clean architecture", "Real-world code patterns"],
            difficulty: "Beginner",
